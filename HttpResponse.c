...
{
    Debug("准备回复，即将调用发送函数");
    if (response == NULL || sendBuf == NULL || socket < 0)
    {
        Debug("参数错误：response/sendBuf/socket为空");
        return;
    }
    if (response->filename == NULL || response->sendDataFunc == NULL)
    {
        Debug("响应参数错误：filename或sendDataFunc为空");
        if (response->filename == NULL)
        {
            Debug("响应参数错误：filename为空");
        }
        if (response->sendDataFunc == NULL)
         {
            Debug("响应参数错误：sendDataFunc为空");
        }
        return;
    }
    //状态行 
    ...
    for(int i=0;i<response->headerNum;i++)
    {
        ...
        Debug("%s: %s",response->headers[i].key,response->headers[i].value);
        bufferAppendString(sendBuf,tmp);
    }
    //空行
